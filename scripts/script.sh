...
send_prompt_to_chatgpt() {
    curl -s -X POST "https://api.openai.com/v1/chat/completions" \
        ...
        -d "{\"model\": \"gpt-3.5-turbo\", \"messages\": $MESSAGES_JSON, \"max_tokens\": 100}"
}
...
INSTRUCTIONS="Based on the description below, please generate a concise JSON object where the keys represent file paths and the values are the corresponding code snippets for a production-ready application. The response should be a valid strictly JSON object without any additional formatting, markdown, or characters outside the JSON structure."
